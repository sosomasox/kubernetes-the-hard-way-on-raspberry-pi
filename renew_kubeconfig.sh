...
echo "---> Renew kube-controller-manager kubeconfig"
for i in `seq 1 3`
do
...
echo "---> Renew kube-scheduler kubeconfig"
for i in `seq 1 3`
do
kubectl config set-cluster kubernetes-the-hard-way \
  --certificate-authority=./certs/cacert/kubernetes-ca.pem \
...
echo "---> Renew admin user kubeconfig"
kubectl config set-cluster kubernetes-the-hard-way \
  --certificate-authority=./certs/cacert/kubernetes-ca.pem \
  --embed-certs=true \
  --server=https://192.168.114.10:64430 \
...
echo "---> Renew kube-proxy kubeconfig"
kubectl config set-cluster kubernetes-the-hard-way \
  --certificate-authority=./certs/cacert/kubernetes-ca.pem \
  --embed-certs=true \
  --server=https://192.168.114.10:64430 \
